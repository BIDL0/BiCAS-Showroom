...
# distributed LICENSE file that is a part of the archive of BiCAS.
# ------------------------------------------------------------------------------
#
#                           (c) 2015-2017, Michal Bidlo
...
color = [ black, yellow, white, red, blue, green, brown, orange, cyan, violet,
          gray, magenta, pink ]
...
class CA:
    ...
    def show_help(self, win):
        ...
        self.draw_text(win, "The .cas files specify various initial")
        self.draw_text(win, "  states for CA case studies included.")
    ...
    def draw(self, win):
        ...
        offx = (_WIN - self.cell_w*self.cols) / 2
        offy = (_WIN - self.cell_h*self.rows) / 2
        for row in range(1, self.rows+1):
            for col in range(1, self.cols+1):
                pygame.draw.rect(win, color[self.get_cell(row, col)],
                    ((col-1)*self.cell_w+offx, (row-1)*self.cell_h+offy,
                    self.cell_w, self.cell_h), 0)
                pygame.draw.rect(win, blue, ((col-1)*self.cell_w+offx,
                    (row-1)*self.cell_h+offy, self.cell_w, self.cell_h), 1)
...
#                        ca.develop(win)
        ...
        if devel == 1:
            ca.develop(win)
            # if you wish to slow down the CA development, it is possible to
            # influence it here using the 4th integer argument running BiCAS.
            pygame.time.wait(ca.delay_ms)
